...
function WSSecurity(username, password, passwordType, excludeTimestamp) {
    this._username = username;
    this._password = password;
    this._passwordType = passwordType || 'PasswordText';
    this._excludeTimestamp = !!excludeTimestamp;
}
...
/* Security based on a session cookie
The service endpoing will do a Set-Cookie on login. Future calls will use this cookie.
It is also possible to manage the cookies by using the client.security.getCookie and .setCookie
functions
*/
function CookieSecurity(name, def) {
    this._name = name;
    this._value = def || "";
    this._rgx = new RegExp(name + "=([^;]*);");
}

CookieSecurity.prototype.addHeaders = function (headers) {
    if (this._value) {
        headers['Cookie'] = this._name + '=' + this._value;
    }
}

CookieSecurity.prototype.parseHeaders = function(headers) {
    if (headers['set-cookie']) {
        var m = (headers['set-cookie']+'').match(this._rgx);
        if (m) {
            this._value = m[1];
        }
    }
...
CookieSecurity.prototype.getCookie = function () {
    return this._value;
}

CookieSecurity.prototype.setCookie = function (value) {
    this._value = value;
}
...
exports.CookieSecurity = CookieSecurity;
